    ...
    <div className="fixed inset-0 bg-slate-900/90 z-[70] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-2xl rounded-[40px] border-4 border-slate-900 shadow-[20px_20px_0px_0px_rgba(15,23,42,1)] flex flex-col max-h-[95vh] overflow-hidden relative">
        ...
        <div className="p-6 border-b-4 border-slate-100 flex justify-between items-center bg-slate-50/50">
          ...
          <button onClick={onClose} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border-2 border-slate-200 text-slate-400 hover:text-red-600 transition-all">
        ...
        <div className="p-6 bg-slate-50 border-t-4 border-slate-100 flex gap-3 mt-auto">
          <button onClick={onClose} className="flex-1 py-4 rounded-2xl border-2 border-slate-200 text-slate-400 font-black uppercase text-[10px] hover:bg-slate-100 transition-all">Descartar</button>
          <button onClick={onSave} className="flex-[2] py-4 bg-blue-600 hover:bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95"><Check size={20} strokeWidth={4} /> Lançar Tarefa Agora</button>
        </div>
      </div>
    </div>
  );
