...
import type { PricingMarginRule, Profile, Subtask, TaskPriority, UserRole } from "@/lib/types";
...
export function CreateTaskModal({
  ...
  const monthlyWeekdayRepeats = parseMonthlyWeekdayRepeats(selectedDays.join(','));
  const isMonthlyWeekday = isMonthly && monthlyWeekdayRepeats.length > 0;
  ...
  const toggleMonthlyWeekdayRepeat = (ordinal: (typeof MONTHLY_WEEKDAY_ORDINALS)[number]['value'], weekday: string) => {
    ...
    const currentValues = parseMonthlyWeekdayRepeats(selectedDays.join(',')).map((repeat) => repeat.value);
    setSelectedDays(currentValues.includes(value) ? currentValues.filter((item) => item !== value) : [...currentValues, value]);
  };
            ...
            {showMarginFlowFields && (
              <MarginFlowTaskFields
                ...
                onSelectionChange={(data) => {
                  ...
                  if (nextTitle) setTaskTitle(nextTitle);
                }}
              ...
              {isOneOff ? (
                ...
                <div className="space-y-2">
                  <div className="grid grid-cols-2 gap-2 rounded-xl border-2 border-slate-100 bg-slate-50 p-1">
                    ...
                    <button type="button" onClick={() => setSelectedDays([buildMonthlyWeekdayRepeat("1", "seg")])} className={`rounded-lg py-2 text-[9px] font-black uppercase transition ${isMonthlyWeekday ? "bg-white text-blue-600 shadow-sm" : "text-slate-400"}`}>Dia da semana</button>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {TASK_PRIORITY_OPTIONS.map((option) => (
                    ...
                    >
                      ...
                      <span className={`mt-0.5 block text-[8px] font-black uppercase ${taskPriority === option.value ? "text-white/75" : "text-slate-300"}`}>
                        {option.description}
                      </span>
                    </button>
                  ))}
