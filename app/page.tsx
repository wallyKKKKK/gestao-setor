...
// --- 1. UTILITÁRIOS SEGUROS (NUNCA QUEBRAM) ---
const getTodayStr = () => {
  const d = new Date();
...
const getLastOccurrence = (task: any) => {
  ...
  const start = new Date(task.created_at || new Date());
  ...
  for (let w = 0; w < 52; w++) {
    ...
    const nextW = new Date(startMon); nextW.setDate(startMon.getDate() + ((w + 1) * 7));
    if (nextW.toISOString().split('T')[0] > today) break;
  }
  return lastFound;
};
...
export default function App() {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        ...
        supabase.from('profiles').select('role, full_name').eq('id', session.user.id).single()
          .then(({ data }) => { if (data) { setUserRole(data.role); setNewName(data.full_name || ''); } });
        loadData();
      }
    });
  }, [loadData]);

  // --- LOGICA DE FILTRAGEM BLINDADA ---
  ...
  const stats = useMemo(() => {
    ...
    return { total: relevant.length, done, perc: relevant.length > 0 ? Math.round((done/relevant.length)*100) : 0 };
  ...
  const addTask = async () => {
    if (!taskTitle) return;
    const isRec = selectedDays.length > 0;
    const { error } = await supabase.from('tasks').insert([{ 
      title: taskTitle.toUpperCase(), assigned_to: assignedTo, status: 'pendente', category, notes, 
      repeat_days: isRec ? selectedDays.join(',') : "", repeat_interval: isRec ? repeatInterval : 1,
      subtasks: tempSubtasks, due_date: isRec ? null : getTodayStr() 
    }]);
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 font-sans ${isDarkMode ? 'bg-[#0B0E14] text-slate-100' : 'bg-[#F9FAFB] text-slate-900'} overflow-x-hidden w-full`}>
      
      {/* NAVBAR */}
      <nav className={`sticky top-0 z-50 border-b backdrop-blur-md transition-all ${isDarkMode ? 'bg-[#11141D]/90 border-white/5' : 'bg-white/90 border-slate-200'}`}>
        <div className="max-w-7xl mx-auto px-6 h-14 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 rounded-xl text-slate-400 hover:text-indigo-500">
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className={`flex p-0.5 rounded-xl ring-1 overflow-x-auto no-scrollbar max-w-full ${isDarkMode ? 'bg-black/20 ring-white/5' : 'bg-slate-200/50 ring-slate-200'}`}>
              {categories.map(tab => (
                <button key={tab} onClick={() => { setActiveTab(tab); setShowCreateBox(false); }} className={`px-4 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-indigo-500'}`}>{tab}</button>
              ))}
            </div>
          </div>

          <div className="flex items-center justify-center gap-2 overflow-x-auto no-scrollbar pb-1">
            <button onClick={() => setFilterUser('Todos')} className={`px-3 py-1 rounded-lg text-[8px] font-bold uppercase border transition-all ${filterUser === 'Todos' ? 'bg-slate-900 text-white dark:bg-indigo-600' : 'bg-white border dark:bg-white/5 dark:text-slate-400'}`}>Todos</button>
            {profiles.map(p => (
      ...
      <main className="max-w-4xl mx-auto p-6">
        {activeTab === 'DASHBOARD' ? (
          <div className="space-y-6 animate-in fade-in duration-700 text-center">
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <DashboardCard label="Metas" val={stats.total} isDark={isDarkMode} />
                <DashboardCard label="Sucesso" val={stats.done} color="text-emerald-500" isDark={isDarkMode} />
                <DashboardCard label="Pendente" val={stats.pendentes} color="text-rose-500" isDark={isDarkMode} />
             </div>
             <div className={`p-16 rounded-[48px] border transition-all ${isDarkMode ? 'bg-[#11141D] border-white/5 shadow-none' : 'bg-white border-slate-100 shadow-xl'}`}>
                <h3 className={`text-8xl font-black tracking-tighter mb-4 ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>{stats.perc}%</h3>
                ...
                <div className="mt-8 max-w-sm mx-auto bg-slate-100 dark:bg-white/5 h-2 rounded-full overflow-hidden">
                  <div className="bg-indigo-600 h-full transition-all duration-1000 shadow-[0_0_15px_rgba(79,70,229,0.6)]" style={{ width: `${stats.perc}%` }} />
                </div>
             </div>
          </div>
          ...
          <div className="mt-8 space-y-6 max-w-3xl mx-auto">
             {userRole === 'admin' && (
               <div className={`p-8 rounded-[32px] border ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-200'}`}>
                  <input className={`w-full p-4 rounded-xl border mb-4 font-bold text-sm ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'}`} placeholder="Assunto..." value={newAnnounce.title} onChange={e => setNewAnnounce({...newAnnounce, title: e.target.value})} />
                  <textarea className={`w-full p-4 rounded-xl border mb-6 text-sm font-medium ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'}`} placeholder="Mensagem..." rows={3} value={newAnnounce.content} onChange={e => setNewAnnounce({...newAnnounce, content: e.target.value})} />
                  <button onClick={async () => { await supabase.from('announcements').insert([{ ...newAnnounce, author_id: user.id }]); setNewAnnounce({title:'', content:''}); loadData(); }} className="w-full bg-indigo-600 text-white p-4 rounded-xl font-bold text-xs uppercase tracking-widest">Publicar Agora</button>
               </div>
             )}
             {announcements.map(a => (
               <div key={a.id} className={`p-8 rounded-[32px] border-l-8 border-indigo-600 shadow-xl ${isDarkMode ? 'bg-[#11141D] border-white/5 text-white' : 'bg-white border-slate-100 text-slate-950'}`}>
                 <h4 className="text-xl font-bold mb-3 uppercase italic">{a.title}</h4>
                 <p className="text-sm font-medium opacity-70 mb-4">{a.content}</p>
                 <div className="text-[8px] font-bold opacity-30 uppercase tracking-widest">{new Date(a.created_at).toLocaleString()}</div>
               </div>
             ))}
          </div>
        ) : (
          <>
            <div className="mb-10 flex justify-center">
              <button onClick={() => setShowCreateBox(!showCreateBox)} className={`flex items-center gap-3 px-10 py-3.5 rounded-full font-bold text-[10px] uppercase tracking-widest transition-all ${showCreateBox ? 'bg-rose-500 text-white shadow-xl' : 'bg-indigo-600 text-white shadow-2xl hover:scale-105 active:scale-95'}`}>
            ...
            {showCreateBox && (
              <div className={`p-10 rounded-[40px] border mb-16 animate-in slide-in-from-top-10 duration-500 shadow-2xl ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-100'}`}>
                ...
                <textarea className={`w-full p-6 rounded-2xl mb-8 font-medium text-lg border transition-all ${isDarkMode ? 'bg-white/5 border-white/10 focus:bg-white/10' : 'bg-slate-50 border-slate-100 focus:bg-white shadow-inner text-slate-900'}`} placeholder="Instruções e notas técnicas..." rows={2} value={notes} onChange={e => setNotes(e.target.value)} />
                
                {/* SUBTAREFAS NA CRIAÇÃO */}
                <div className="bg-indigo-600/5 p-6 rounded-[30px] border border-indigo-600/10 mb-10 text-left">
                   <label className="text-[10px] font-bold uppercase text-indigo-600 mb-4 block flex items-center gap-2"><ListChecks size={14}/> Checklist Estratégico</label>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     {tempSubtasks.map((sub, idx) => (
                       <div key={idx} className="flex items-center gap-3 bg-white dark:bg-black/20 p-3 rounded-xl border">
                         <input className={`flex-1 bg-transparent font-bold text-xs outline-none ${isDarkMode ? 'text-white' : 'text-slate-950'}`} value={sub.title} onChange={e => { const n = [...tempSubtasks]; n[idx].title = e.target.value; setTempSubtasks(n); }} placeholder="Definir passo..." />
                         <button onClick={() => setTempSubtasks(tempSubtasks.filter((_, i) => i !== idx))} className="text-rose-400"><X size={18}/></button>
                       </div>
                     ))}
                     <button onClick={() => setTempSubtasks([...tempSubtasks, {title: '', done: false}])} className="flex items-center justify-center gap-2 p-3 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 font-bold text-[9px] hover:border-indigo-600 transition-all uppercase">+ Novo Passo</button>
                   </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end border-t dark:border-white/5 pt-10 text-left">
                   <div className="space-y-4">
                      <label className="text-[10px] font-bold uppercase text-slate-400 ml-1">Responsável</label>
                      <select disabled={userRole === 'membro'} className="w-full p-4 rounded-xl bg-indigo-600 text-white font-bold text-xs border-none shadow-xl cursor-pointer" value={assignedTo} onChange={e => setAssignedTo(e.target.value)}>{profiles.map(p => <option key={p.id} value={p.id} className="text-black">{p.full_name}</option>)}</select>
                   </div>
                   <div className="md:col-span-1 space-y-5 text-center">
                      <div className="flex gap-2">
                        <button onClick={() => setIsPontual(!isPontual)} className={`flex-1 p-4 rounded-2xl font-bold uppercase text-[9px] border transition-all ${isPontual ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>{isPontual ? '⚡ Única' : '🔄 Recorrente'}</button>
                        {!isPontual && <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10"><Repeat size={14} className="text-indigo-500"/><input type="number" min="1" className={`w-8 bg-transparent font-bold text-sm text-center ${isDarkMode ? 'text-white' : 'text-slate-950'}`} value={repeatInterval} onChange={e => setRepeatInterval(parseInt(e.target.value))} /></div>}
                      </div>
                      {!isPontual && <div className={`flex gap-1 p-1 rounded-lg border ${isDarkMode ? 'bg-black/30 border-white/5' : 'bg-slate-100 border-slate-200'}`}>{weekDays.map(day => (<button key={day.id} type="button" onClick={() => toggleDay(day.id)} className={`flex-1 h-9 rounded-lg font-bold text-[9px] transition-all ${selectedDays.includes(day.id) ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-600'}`}>{day.label}</button>))}</div>}
                   </div>
                   <button onClick={addTask} className="w-full bg-slate-900 dark:bg-indigo-600 text-white p-5 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-2xl transition-all hover:scale-105 active:scale-95">Lançar no Sistema</button>
                </div>
              </div>
            )}

            <div className="space-y-4">
              <h2 className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.5em] flex items-center gap-4 mb-8">
              ...
              {filteredTasks.map(task => {
                ...
                return (<TaskBox key={task.id} task={task} profiles={profiles} isLate={isLate} isDoneToday={isDone} userRole={userRole} currentUserId={user.id} isDarkMode={isDarkMode} onToggle={() => handleToggle(task)} onEdit={(t: any) => { setEditingTask(t); setShowEditModal(true); }} onUpdate={loadData} />)
              })}
            </div>
          </>
        )}
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-[70] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
          <div className={`p-10 rounded-[40px] w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[90vh] ${isDarkMode ? 'bg-[#11141D] border border-white/5' : 'bg-white'}`}>
             ...
             <div className="space-y-6 text-left">
                <input className={`w-full text-2xl font-bold bg-transparent outline-none border-b uppercase ${isDarkMode ? 'text-white' : 'text-slate-950'}`} value={editingTask.title} onChange={e => setEditingTask({...editingTask, title: e.target.value})} />
                <textarea className={`w-full p-6 border rounded-3xl font-bold transition-all ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-100 text-slate-900'}`} rows={3} value={editingTask.notes || ''} onChange={e => setEditingTask({...editingTask, notes: e.target.value})} />
                ...
                <div className="grid grid-cols-2 gap-4">
                   <select className={`w-full p-4 rounded-xl font-bold text-xs ${isDarkMode ? 'bg-white/5 text-white' : 'bg-white border-slate-200 text-slate-950'}`} value={editingTask.assigned_to} onChange={e => setEditingTask({...editingTask, assigned_to: e.target.value})}>{profiles.map(p => <option key={p.id} value={p.id} className="text-black">{p.full_name}</option>)}</select>
                   <input type="number" className={`w-full p-4 rounded-xl font-bold text-xs ${isDarkMode ? 'bg-white/5 text-white' : 'bg-slate-100 border-slate-200 text-slate-950'}`} value={editingTask.repeat_interval || 1} onChange={e => setEditingTask({...editingTask, repeat_interval: parseInt(e.target.value)})} />
                </div>
                
                <div className="space-y-4">
                  <div className={`flex gap-1 p-1.5 rounded-xl border ${isDarkMode ? 'bg-black/30 border-white/5' : 'bg-slate-100 border-slate-200'}`}>
                ...
                <button onClick={updateTask} className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 transition-all mt-4">Salvar Alterações</button>
...
function TaskBox({ task, profiles, isLate, isDoneToday, onToggle, onEdit, onUpdate, userRole, currentUserId, isDarkMode }: any) {
  ...
  return (
    <div className={`p-5 rounded-[32px] border transition-all duration-500 flex flex-col gap-4 relative group ${
      isDoneToday 
      ? 'bg-slate-50 dark:bg-black/20 border-transparent opacity-50 grayscale shadow-none text-left' 
      : isLate 
        ? 'bg-rose-50/70 dark:bg-rose-950/20 border-rose-300 dark:border-rose-500/30 shadow-xl shadow-rose-200/40 text-left' 
      ...
      <div className="flex items-center gap-5 text-left">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <h3 className={`text-xl font-black tracking-tight leading-tight transition-all uppercase ${isDoneToday ? 'text-slate-400 line-through' : isLate ? 'text-rose-600' : 'text-slate-950 dark:text-white'}`}>{task.title}</h3>
          
          {subTotal > 0 && (
            <div className="flex items-center gap-3 mt-1.5">
              <div className="flex-1 bg-slate-100 dark:bg-white/5 h-1.5 rounded-full overflow-hidden border dark:border-white/5"><div className="bg-indigo-600 h-full transition-all duration-700 shadow-[0_0_10px_rgba(79,70,229,0.5)]" style={{ width: `${(subDone / subTotal) * 100}%` }} /></div>
              <span className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">{subDone}/{subTotal}</span>
              {expanded ? <ChevronUp size={14} className="text-indigo-600"/> : <ChevronDown size={14} className="text-slate-300"/>}
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 mt-4 font-sans">
            <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider flex items-center gap-1.5 border transition-all ${isDoneToday ? 'bg-slate-100 border-transparent text-slate-300' : 'bg-slate-900 text-white border-slate-800'}`}><User size={10} className="text-indigo-500"/> {profiles.find(p => p.id === task.assigned_to)?.full_name || 'Agente'}</span>
            <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border transition-all ${isDoneToday ? 'bg-slate-100 border-transparent text-slate-300' : 'bg-indigo-50 border-indigo-100 text-indigo-600'}`}>{task.category}</span>
            <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border transition-all ${isDoneToday ? 'bg-slate-100 border-transparent text-slate-300' : 'bg-white border-slate-200 text-slate-500'}`}><Calendar size={10}/> Próxima: {getNextOccurrence(task)}</span>
          </div>
        </div>
        {canModify && (
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => onEdit(task)} className="p-2 text-slate-400 hover:text-indigo-600 rounded-lg hover:bg-indigo-50 dark:hover:bg-white/5 transition-all"><Edit3 size={18}/></button>
            <button onClick={async () => { if(confirm('Excluir?')) { await supabase.from('tasks').delete().eq('id', task.id); onUpdate(); } }} className="p-2 text-slate-400 hover:text-rose-500 rounded-lg hover:bg-rose-50 dark:hover:bg-white/5 transition-all"><Trash2 size={18}/></button>
          </div>
        )}
      </div>
...
function DashboardCard({ label, val, color, isDark }: any) {
  return (<div className={`p-8 rounded-[40px] border transition-all ${isDark ? 'bg-[#161B22] border-white/5 shadow-none' : 'bg-white border-slate-100 shadow-2xl shadow-slate-200/30'}`}><span className="text-[9px] font-black uppercase tracking-[0.3em] block mb-2 opacity-40 italic">{label}</span><span className={`text-7xl font-black tracking-tighter ${color || (isDark ? 'text-white' : 'text-slate-950')}`}>{val}</span></div>)
}
...
function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#0F111A] flex items-center justify-center p-6 text-center">
      <div className="bg-white p-12 rounded-[60px] w-full max-w-sm shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative overflow-hidden">
        ...
        <h1 className="text-6xl font-black italic tracking-tighter text-slate-950 leading-none mb-12 uppercase">SUPPLY<br/><span className="text-indigo-600 text-3xl not-italic tracking-[0.3em] font-medium opacity-90 uppercase">Tasker</span></h1>
        <div className="space-y-4">
          ...
          <button onClick={processAuth} className="w-full bg-slate-950 text-white p-6 rounded-[30px] font-black uppercase text-xs tracking-widest shadow-2xl hover:bg-indigo-600 transition-all mt-6">{isSignUp ? 'Criar Identidade' : 'Acessar Central'}</button>
          <button onClick={() => setIsSignUp(!isSignUp)} className="w-full mt-8 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-indigo-600 transition-colors uppercase font-sans">Solicitar Acesso</button>
        </div>
      </div>
    </div>
  )
