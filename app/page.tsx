...
import { 
  ...
  Edit3, ChevronRight, Activity, ListChecks, ChevronDown, ChevronUp, FileText, Megaphone
} from 'lucide-react'
...
export default function App() {
  ...
  const stats = (() => {
  // Usamos as tarefas já filtradas pela regra de classe para o cálculo
  // Isso garante que o Membro veja os SEUS stats e o Gerente veja os stats da EQUIPE.
  const baseTasks = filteredTasks; 

  const total = baseTasks.length;
  const done = baseTasks.filter(t => {
    const s = getLastOccurrence(t); 
    const d = t.last_done_date || '1970-01-01';
    return d >= s;
  }).length;

  const pendentes = total - done;
  const porcentagem = total > 0 ? Math.round((done / total) * 100) : 0;

  return { total, concluidas: done, pendentes, porcentagem };
})();
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 pb-20 font-sans overflow-x-hidden w-full">
      <nav className="bg-[#0F172A] text-white sticky top-0 z-30 shadow-2xl border-b border-white/10 px-6 h-20 flex justify-between items-center">
        ...
        <h1 className="text-xl font-black italic tracking-tighter uppercase">WALLY<span className="text-blue-500 text-sm block not-italic font-medium">Task Manager</span></h1></div>
