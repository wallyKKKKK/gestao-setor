...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${isDarkMode ? 'bg-[#0B0E14] text-slate-100' : 'bg-[#F8FAFC] text-slate-900'} pb-20 font-sans overflow-x-hidden w-full`}>
      
      {/* NAVBAR SIMPLIFICADA */}
      <nav className={`sticky top-0 z-50 border-b transition-all ${isDarkMode ? 'bg-[#11141D] border-white/5 shadow-2xl' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-6 h-14 flex justify-between items-center">
          <div className="flex items-center gap-2.5">
            <div className="bg-indigo-600 p-1.5 rounded-lg"><Activity size={18} className="text-white" /></div>
            <h1 className={`text-sm font-bold tracking-tight uppercase ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>Supply<span className="text-indigo-600">Builder</span></h1>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 transition-all text-slate-400">{isDarkMode ? <Sun size={18}/> : <Moon size={18}/>}</button>
            <button onClick={() => setShowProfileModal(true)} className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-full border transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
              <div className="w-6 h-6 bg-indigo-600 rounded-full flex items-center justify-center text-[9px] font-bold text-white shadow-md">{newName?.charAt(0) || 'U'}</div>
              <span className="text-[10px] font-bold uppercase tracking-wider hidden sm:block">{newName || 'Perfil'}</span>
            </button>
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className={`flex p-0.5 rounded-xl ring-1 overflow-x-auto no-scrollbar max-w-full ${isDarkMode ? 'bg-black/20 ring-white/5' : 'bg-slate-200/50 ring-slate-200'}`}>
              {categories.map(tab => (
                <button key={tab} onClick={() => { setActiveTab(tab); setShowCreateBox(false); }} className={`px-4 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider transition-all whitespace-nowrap ${activeTab === tab ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-indigo-500'}`}>{tab}</button>
              ))}
            </div>
          </div>
          <div className="flex items-center justify-center gap-2 overflow-x-auto no-scrollbar pb-1">
            <button onClick={() => setFilterUser('Todos')} className={`px-3 py-1 rounded-lg text-[8px] font-bold uppercase border transition-all ${filterUser === 'Todos' ? 'bg-slate-900 text-white dark:bg-indigo-600 border-transparent shadow-md' : 'bg-white border-slate-200 dark:bg-white/5 dark:text-slate-400'}`}>Todos</button>
            {profiles.map(p => (
              <button key={p.id} onClick={() => setFilterUser(p.id)} className={`px-3 py-1 rounded-lg text-[8px] font-bold uppercase border transition-all flex items-center gap-2 ${filterUser === p.id ? 'bg-indigo-600 text-white border-transparent shadow-lg' : 'bg-white border-slate-200 dark:bg-white/5 dark:text-slate-500'}`}>{p.full_name?.split(' ')[0]}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto p-6">
        {activeTab === 'DASHBOARD' ? (
          <div className="space-y-6 animate-in fade-in duration-700 text-center">
             ...
             <div className={`p-10 rounded-[32px] border transition-all ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/20'}`}>
                <h3 className={`text-7xl font-bold tracking-tighter mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{stats.porcentagem}%</h3>
                <p className="text-[10px] font-bold uppercase text-slate-400 tracking-[0.4em]">Eficiência Operacional</p>
                <div className="mt-8 max-w-sm mx-auto bg-slate-100 dark:bg-white/5 h-2 rounded-full overflow-hidden">
                  <div className="bg-indigo-600 h-full transition-all duration-1000 shadow-[0_0_15px_rgba(79,70,229,0.6)]" style={{ width: `${stats.porcentagem}%` }} />
                </div>
             </div>
          </div>
          ...
          <div className="mt-4 space-y-6 max-w-2xl mx-auto">
             {userRole === 'admin' && (
               <div className={`p-6 rounded-[24px] border ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-200 shadow-lg'}`}>
                  <input className={`w-full p-3 rounded-xl border mb-3 font-bold text-sm ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`} placeholder="Título do Alerta" value={newAnnounce.title} onChange={e => setNewAnnounce({...newAnnounce, title: e.target.value})} />
                  <textarea className={`w-full p-3 rounded-xl border mb-4 text-sm font-medium ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-200 text-slate-900'}`} placeholder="Mensagem..." rows={2} value={newAnnounce.content} onChange={e => setNewAnnounce({...newAnnounce, content: e.target.value})} />
                  <button onClick={async () => { await supabase.from('announcements').insert([{ ...newAnnounce, author_id: user.id }]); setNewAnnounce({title:'', content:''}); fetchAnnouncements(); }} className="w-full bg-indigo-600 text-white p-3 rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all">Publicar Agora</button>
               </div>
             )}
             {announcements.map(a => (
               <div key={a.id} className={`p-6 rounded-[32px] border-l-8 border-indigo-600 shadow-xl ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-100'}`}>
                 <h4 className="text-xl font-bold mb-2 uppercase tracking-tight">{a.title}</h4>
                 <p className="text-sm font-medium opacity-70 mb-4">{a.content}</p>
                 <div className="text-[8px] font-bold uppercase opacity-30">Postado: {new Date(a.created_at).toLocaleString()}</div>
               </div>
             ))}
          </div>
        ) : (
          <>
            <div className="mb-8 flex justify-center">
              <button onClick={() => setShowCreateBox(!showCreateBox)} className={`flex items-center gap-3 px-8 py-3 rounded-full font-bold text-[10px] uppercase tracking-widest transition-all ${showCreateBox ? 'bg-slate-200 dark:bg-white/10 text-slate-500' : 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/20 hover:scale-105'}`}>
                {showCreateBox ? <X size={14}/> : <Plus size={14}/>} {showCreateBox ? 'Cancelar' : 'Nova Missão'}
              </button>
            </div>

            {showCreateBox && (
              <div className={`p-8 rounded-[32px] border mb-12 animate-in slide-in-from-top-4 duration-500 shadow-2xl ${isDarkMode ? 'bg-[#11141D] border-white/5' : 'bg-white border-slate-100'}`}>
                <input className={`w-full text-2xl font-bold bg-transparent outline-none mb-6 placeholder-slate-200 uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`} placeholder="Nome da Missão" value={taskTitle} onChange={e => setTaskTitle(e.target.value)} />
                <textarea className={`w-full p-4 rounded-xl border mb-6 text-sm font-medium ${isDarkMode ? 'bg-white/5 border-white/5 text-white' : 'bg-slate-50 border-slate-100 text-slate-900 focus:bg-white'}`} placeholder="Instruções de Campo..." rows={2} value={notes} onChange={e => setNotes(e.target.value)} />
                
                <div className="bg-indigo-600/5 p-5 rounded-2xl border border-indigo-600/10 mb-8">
                   <label className="text-[9px] font-bold uppercase text-indigo-600 mb-3 block flex items-center gap-2"><ListChecks size={14}/> Checklist de Passos</label>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                     {tempSubtasks.map((sub, idx) => (
                       <div key={idx} className="flex items-center gap-2 bg-white dark:bg-white/5 p-2.5 rounded-xl border">
                         <input className={`flex-1 bg-transparent font-medium text-xs outline-none ${isDarkMode ? 'text-white' : 'text-slate-900'}`} value={sub.title} onChange={e => { const n = [...tempSubtasks]; n[idx].title = e.target.value; setTempSubtasks(n); }} placeholder="Passo..." />
                         <button onClick={() => setTempSubtasks(tempSubtasks.filter((_, i) => i !== idx))} className="text-rose-400"><X size={16}/></button>
                       </div>
                     ))}
                     <button onClick={() => setTempSubtasks([...tempSubtasks, {title: '', done: false}])} className="flex items-center justify-center gap-2 p-2 border-2 border-dashed rounded-xl text-slate-400 font-bold text-[9px] hover:border-indigo-600 transition-all uppercase">+ Adicionar Etapa</button>
                   </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end border-t dark:border-white/5 pt-8 text-left">
                   <div className="space-y-2">
                      <label className="text-[9px] font-bold uppercase text-slate-400 ml-1 tracking-widest">Responsável</label>
                   ...
                   <div className="space-y-2">
                      <div className="flex gap-2 mb-2">
                        <button onClick={() => setIsPontual(!isPontual)} className={`flex-1 p-3 rounded-xl font-bold uppercase text-[8px] border transition-all ${isPontual ? 'bg-amber-500 text-white border-transparent shadow-md' : 'bg-slate-100 dark:bg-white/5 text-slate-400'}`}>{isPontual ? '⚡ Única' : '🔄 Recorrente'}</button>
                        {!isPontual && <div className="flex items-center gap-2 px-3 py-1 rounded-xl bg-slate-100 dark:bg-white/5 border"><Repeat size={12} className="text-indigo-500"/><input type="number" min="1" className={`w-8 bg-transparent font-bold text-sm text-center ${isDarkMode ? 'text-white' : 'text-slate-900'}`} value={repeatInterval} onChange={e => setRepeatInterval(parseInt(e.target.value))} /></div>}
                      </div>
                      {!isPontual && <div className="flex gap-1 bg-slate-100 dark:bg-white/5 p-1 rounded-lg border">{weekDays.map(day => (<button key={day.id} type="button" onClick={() => toggleDay(day.id)} className={`flex-1 h-8 rounded-lg font-black text-[9px] transition-all ${selectedDays.includes(day.id) ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400'}`}>{day.label}</button>))}</div>}
                   </div>
                   <button onClick={addTask} className="w-full bg-slate-900 dark:bg-white dark:text-black text-white p-4 rounded-xl font-bold uppercase text-[10px] tracking-widest shadow-xl hover:scale-[1.02] transition-all">Lançar no Sistema</button>
                </div>
              </div>
            )}
      ...
      {showEditModal && editingTask && (
        <div className="fixed inset-0 bg-slate-900/60 z-[70] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
          <div className={`p-10 rounded-[40px] w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[90vh] ${isDarkMode ? 'bg-[#11141D] border border-white/5' : 'bg-white'}`}>
             <h2 className="text-xl font-bold mb-8 tracking-tight text-indigo-600 uppercase italic">Editar Operação</h2>
             <div className="space-y-6 text-left">
                ...
                <textarea className={`w-full p-5 rounded-2xl border text-sm font-medium ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50'}`} rows={3} value={editingTask.notes || ''} onChange={e => setEditingTask({...editingTask, notes: e.target.value})} />
                <div className="space-y-4 border-t pt-6 border-slate-100 dark:border-white/5">
                  <label className="text-[10px] font-bold uppercase opacity-40 flex items-center gap-2"><ListChecks size={14}/> Checklist de Passos</label>
                  <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <input type="number" className={`w-full p-4 rounded-xl font-bold text-xs ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200 text-slate-900'}`} value={editingTask.repeat_interval || 1} onChange={e => setEditingTask({...editingTask, repeat_interval: parseInt(e.target.value)})} />
                </div>
                <div className="space-y-4">
                  <div className={`flex gap-1 p-1.5 rounded-xl border ${isDarkMode ? 'bg-black/30 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                ...
                <button onClick={updateTask} className="w-full bg-indigo-600 text-white p-5 rounded-2xl font-bold uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 transition-all mt-4">Confirmar Mudanças</button>
             </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-5 text-left">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          ...
          {subTotal > 0 && (
            <div className="flex items-center gap-3 mt-1.5">
              ...
              <span className="text-[7px] font-bold text-slate-400 uppercase tracking-widest">{subDone}/{subTotal} ETAPAS</span>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            <span className={`px-2 py-1 rounded-lg text-[8px] font-bold uppercase flex items-center gap-1.5 border transition-all ${isDoneToday ? 'bg-slate-100 border-transparent text-slate-300' : 'bg-slate-900 text-white border-slate-800'}`}><User size={10}/> {profiles.find(p => p.id === task.assigned_to)?.full_name || 'Agente'}</span>
        ...
        {canModify && (
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => onEdit(task)} className="p-2 text-slate-400 hover:text-indigo-600 transition-all"><Edit3 size={18}/></button>
            <button onClick={async () => { if(confirm('Excluir?')) { await supabase.from('tasks').delete().eq('id', task.id); onUpdate(); } }} className="p-2 text-slate-400 hover:text-rose-500 transition-all"><Trash2 size={18}/></button>
          </div>
        )}
      </div>
...
function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#0F111A] flex items-center justify-center p-6 text-center font-sans">
      <div className="bg-white p-12 rounded-[48px] w-full max-w-sm shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative overflow-hidden">
        ...
        <h1 className="text-5xl font-bold tracking-tighter text-slate-950 leading-none mb-10">WALLY<br/><span className="text-indigo-600">Tasker</span></h1>
        <div className="space-y-4">
          ...
          <button onClick={() => setIsSignUp(!isSignUp)} className="w-full mt-6 text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:text-indigo-600">Solicitar Acesso</button>
        </div>
      </div>
    </div>
  )
