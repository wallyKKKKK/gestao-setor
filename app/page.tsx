...
// --- 1. UTILITÁRIOS DE DATA (Fuso Horário e Ciclos) ---
const getTodayStr = () => {
  const date = new Date();
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 font-sans ${isDarkMode ? 'bg-[#0B0E14] text-slate-100' : 'bg-[#F9FAFB] text-slate-900'} overflow-x-hidden w-full`}>
      
      {/* NAVBAR */}
      <nav className={`sticky top-0 z-50 border-b backdrop-blur-xl transition-all ${isDarkMode ? 'bg-[#11141D]/80 border-white/5 shadow-2xl' : 'bg-white/80 border-slate-200 shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-6 h-14 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-600 p-1.5 rounded-lg shadow-lg"><Activity size={18} className="text-white" /></div>
            <h1 className={`text-sm font-bold tracking-tight uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Supply<span className="text-indigo-600">Builder</span></h1>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => setIsDarkMode(!isDarkMode)} className={`p-2 rounded-xl transition-all ${isDarkMode ? 'bg-white/5 text-yellow-400' : 'bg-slate-100 text-slate-500'}`}>{isDarkMode ? <Sun size={20}/> : <Moon size={20}/>}</button>
            <button onClick={() => setShowProfileModal(true)} className={`flex items-center gap-2 pl-1 pr-3 py-1 rounded-full border transition-all ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
              ...
              <span className={`text-[11px] font-bold uppercase hidden sm:block ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{newName || 'Perfil'}</span>
            </button>
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 space-y-4">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className={`flex p-1 rounded-2xl ring-1 overflow-x-auto no-scrollbar max-w-full ${isDarkMode ? 'bg-black/40 ring-white/5' : 'bg-slate-200/50 ring-slate-200'}`}>
              {categories.map(tab => (
                <button key={tab} onClick={() => { setActiveTab(tab); setShowCreateBox(false); }} className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-indigo-500'}`}>{tab}</button>
              ))}
            </div>
          </div>
        ...
        {activeTab === 'DASHBOARD' ? (
          <div className="space-y-8 animate-in fade-in duration-700 text-center mt-6">
             ...
             <div className={`p-16 rounded-[48px] border transition-all ${isDarkMode ? 'bg-[#161B22] border-white/5 shadow-none' : 'bg-white border-slate-100 shadow-2xl'}`}>
                <h3 className={`text-[120px] font-black tracking-tighter mb-4 leading-none ${isDarkMode ? 'text-white' : 'text-slate-950'}`}>{stats.perc}%</h3>
                <p className="text-xs font-black uppercase text-slate-400 tracking-[0.4em]">Eficiência Operacional</p>
                <div className="mt-12 max-w-lg mx-auto bg-slate-100 dark:bg-white/5 h-4 rounded-full overflow-hidden p-1 shadow-inner"><div className="bg-indigo-600 h-full transition-all duration-1000 shadow-[0_0_30px_rgba(79,70,229,0.8)] rounded-full" style={{ width: `${stats.perc}%` }} /></div>
             </div>
          </div>
          ...
          <>
            ...
            {showCreateBox && (
              <div className={`p-10 rounded-[48px] border mb-16 animate-in slide-in-from-top-10 duration-500 shadow-2xl ${isDarkMode ? 'bg-[#161B22] border-white/5' : 'bg-white border-slate-100'}`}>
                ...
                <div className="bg-indigo-600/5 p-8 rounded-[40px] border border-indigo-600/10 mb-10 text-left">
      ...
      {showEditModal && editingTask && (
        <div className="fixed inset-0 bg-slate-900/70 z-[70] flex items-center justify-center p-4 backdrop-blur-md animate-in zoom-in-95">
          <div className={`p-14 rounded-[60px] w-full max-w-2xl shadow-[0_50px_100px_rgba(0,0,0,0.5)] overflow-y-auto max-h-[95vh] ${isDarkMode ? 'bg-[#161B22]' : 'bg-white'}`}>
             <h2 className="text-4xl font-black uppercase mb-12 tracking-tighter text-indigo-600 italic text-center">Modificar Operação</h2>
             <div className="space-y-10 text-left">
                ...
                <div className="space-y-4 border-t-4 pt-8 border-slate-100 dark:border-white/5">
                  ...
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { const n = [...(editingTask.subtasks || []), {title:'', done:false}]; setEditingTask({...editingTask, subtasks: n}); }} className="w-full py-5 border-4 border-dashed rounded-[35px] text-slate-400 font-black uppercase text-[10px] hover:text-indigo-600 transition-all">+ Adicionar Passo</button>
                  </div>
                </div>
    ...
    <div className={`p-8 rounded-[50px] border transition-all duration-500 flex flex-col gap-6 relative group ${
      ...
      <div className="flex items-center gap-8 text-left">
        ...
        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <h3 className={`text-3xl font-black tracking-tight transition-all leading-none uppercase ${isDoneToday ? 'text-slate-300 dark:text-slate-600 line-through' : isLate ? 'text-rose-600 dark:text-rose-400' : 'text-slate-950 dark:text-white'}`}>{task.title}</h3>
          
          {subTotal > 0 && (
            <div className="flex items-center gap-4 mt-3">
              <div className="flex-1 bg-slate-100 dark:bg-white/5 h-1.5 rounded-full overflow-hidden border dark:border-white/5"><div className="bg-indigo-600 h-full transition-all duration-1000 shadow-[0_0_15px_rgba(79,70,229,0.5)]" style={{ width: `${(subDone / subTotal) * 100}%` }} /></div>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{subDone}/{subTotal} ETAPAS</span>
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            ...
            <span className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase border transition-all ${isDoneToday ? 'bg-slate-100 border-transparent text-slate-300' : 'bg-white border-slate-200 text-slate-500'}`}><Calendar size={10}/> Próxima: {getNextOccurrence(task)}</span>
          </div>
        </div>
        {canModify && (
      ...
      {expanded && subTotal > 0 && (
        <div className="mt-2 space-y-2 border-t border-slate-100 dark:border-white/5 pt-6 animate-in slide-in-from-top-4 duration-500 text-left">
          {subtasks.map((sub: any, idx: number) => (
            <div key={idx} onClick={() => toggleSub(idx)} className={`flex items-center gap-4 p-4 rounded-2xl border transition-all cursor-pointer ${sub.done ? 'bg-emerald-500/5 border-emerald-500/20 text-emerald-600 opacity-60' : 'bg-slate-50 dark:bg-white/5 border-slate-100 dark:border-transparent hover:border-indigo-400'}`}>
              ...
              <span className={`text-sm font-bold uppercase tracking-tighter ${sub.done ? 'line-through' : (isDarkMode ? 'text-white' : 'text-slate-900')}`}>{sub.title}</span>
            </div>
          ))}
        </div>
      )}
...
function DashboardCard({ label, val, color, isDark }: any) {
  return (<div className={`p-10 rounded-[48px] border transition-all ${isDark ? 'bg-[#161B22] border-white/5 shadow-none' : 'bg-white border-slate-100 shadow-2xl shadow-slate-200/30'}`}><span className="text-[10px] font-black uppercase tracking-[0.4em] block mb-4 opacity-30 italic">{label}</span><span className={`text-[120px] font-black tracking-tighter mb-4 leading-none ${color || (isDark ? 'text-white' : 'text-slate-950')}`}>{val}</span></div>)
}
    ...
    <div className="min-h-screen bg-[#0F111A] flex items-center justify-center p-6 text-center">
      <div className="bg-white p-16 rounded-[60px] w-full max-w-sm shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative overflow-hidden">
        ...
        <div className="space-y-4 text-center">
          ...
          <button onClick={processAuth} className="w-full bg-slate-950 text-white p-6 rounded-[30px] font-black uppercase text-xs tracking-widest shadow-2xl hover:bg-indigo-600 transition-all mt-6">{isSignUp ? 'CRIAR IDENTIDADE' : 'ACESSAR CENTRO'}</button>
